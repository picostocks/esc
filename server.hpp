...
//  blk_.unlock();
  ...
  { assert(msg->data!=NULL);
    ...
    if(BANK_MIN_UMASS/2+MESSAGE_FEE(msg->len)>u0.weight){//not enough funds on admin account
      ELOG("ERROR: too low balance txs:%lX+%lX>%lX\n",BANK_MIN_UMASS/2,MESSAGE_FEE(msg->len),u0.weight);
      close(fd);
      return(false);}
    std::map<uint64_t,log_t> log;
...
//#endif
  ...
  { assert(opts_.svid); // READONLY ok
    if(srvs_.nodes[opts_.svid].msid!=msid_){
      ...
      return(0);}
    uint32_t ntime=time(NULL);
    if(ntime-srvs_.now>BLOCKSEC-5){ // use 5s margin
    ...
    read(fd,&u0,sizeof(user_t));
    close(fd);
    if(BANK_MIN_UMASS/2+MESSAGE_FEE((int32_t)txs.size+(int32_t)line.length())>u0.weight){//not enough funds on admin account
      DLOG("ERROR: too low balance txs:%lX+%lX>%lX\n",BANK_MIN_UMASS/2,MESSAGE_FEE((int32_t)txs.size+(int32_t)line.length()),u0.weight);
      return(false);}
